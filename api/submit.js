...
const TABLE_PROJECT  = 'tblQ96Ic8R80d0wH';
...
export default async function handler(req, res) {
  ...
  if (action === 'approve') {
    try {
      ...
      // 1. Create project record (if new project)
      if (body.is_new_project && body.project_number) {
        const projectFields = {
          'Project Number': parseInt(body.project_number),
          'Project Name': body.project_name || '',
          'Project Full description': body.project_number + ' ' + (body.project_name || ''),
          'Account': body.account || body.client_name || '',
        };
        const projectRes = await larkPost(token,
          `/bitable/v1/apps/${APP_TOKEN}/tables/${TABLE_PROJECT}/records`,
          { fields: projectFields }
        );
        if (projectRes.code !== 0) throw new Error('Project create failed: ' + JSON.stringify(projectRes));
      }
      ...
      if (body.contract_start_date) contractFields['Contract Start Date'] = new Date(body.contract_start_date).getTime();
      if (body.contract_end_date) contractFields['Contract End Date'] = new Date(body.contract_end_date).getTime();
      ...
      // 3. Create invoice records for each milestone
